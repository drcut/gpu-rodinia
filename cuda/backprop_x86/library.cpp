...
int *_sync_counter = new int[100000 * 2];
...
void wrapper_barrier(unsigned long long global_idx) {
  ...
  _sync_counter[pingpong_lock * (group_size_x * group_size_y * group_size_z) +
                group_idx]++;
  pthread_mutex_unlock(&lock);
  while (_sync_counter[pingpong_lock *
                           (group_size_x * group_size_y * group_size_z) +
                       group_idx] %
         (BLOCK_DIM_X * BLOCK_DIM_Y * BLOCK_DIM_Z))
    ;
  pingpong_lock = 1 - pingpong_lock;
}
